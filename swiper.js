...
const swiper = function (parentElement, points, imgWidth, isPhone) {
  let ulLeft = [],
    timer;
  ...
  function mPoint() {
    for (let i = 0; i < points.length; i++) {
      ...
      points[i].ontouchstart = function () {
        ...
        pointsFun();
        //添加动画属性
        parentElement.style.transition = "left 0.5s";
        //给point添加选中状态
  ...
  const pointsFun = isPhone ? mPoint : cPoint;
  pointsFun();
  ...
  //监听手指滑动事件
  function mToucMove() {
    let touchstart, touchend, touchtimer;
    ...
    parentElement.addEventListener("touchend", function (event) {
      ...
      let index = Number(event.target.alt);
      if (touchstart != touchend) {
        ...
        if (touchstart > touchend) {
          ...
          if (index > points.length - 1) {
            ...
            parentElement.style.left = ulLeft[index + 1] + "px";
            touchtimer = setTimeout(() => {
              time(index);
              swiper1(index);
            }, 500);
          ...
          if (index < 0) {
            index = 0;
            points[points.length - 1].className += " active";
            //播放图1到图6副本的动画
            parentElement.style.left = ulLeft[0] + "px";
            //将ul的transition属性设置为none,暂时关闭动画效果
            parentElement.style.transition = "none";
